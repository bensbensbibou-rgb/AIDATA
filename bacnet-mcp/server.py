...
class BACnetConfig:
    def __init__(self):
        # Adresse locale et cible configurables via variables d'env pour s'adapter au réseau courant
        self.host = os.getenv("BACNET_MCP_HOST", "192.168.1.177")
        self.port = int(os.getenv("BACNET_MCP_PORT", 47808))
        self.device_instance = int(os.getenv("BACNET_MCP_DEVICE_INSTANCE", 19149))
        self.target_host = os.getenv("BACNET_MCP_TARGET_HOST", "192.168.1.7")
        self.target_port = int(os.getenv("BACNET_MCP_TARGET_PORT", 47808))
        self.bbmd_address = os.getenv("BACNET_MCP_BBMD_ADDRESS", "")
        self.bbmd_ttl = int(os.getenv("BACNET_MCP_BBMD_TTL", 900))
...
async def get_bacnet_app() -> Application:
    ...
    if _bacnet_app is None:
        ...
        try:
            ...
            if settings.bacnet.bbmd_address:
                sys.argv.extend(["--foreign", f"{settings.bacnet.bbmd_address}:{settings.bacnet.bbmd_ttl}"])
...
async def reset_bacnet_app():
    """
    Ferme l'application BACnet existante et force une réinitialisation
    au prochain appel de get_bacnet_app().
    """
    global _bacnet_app
    if _bacnet_app:
        logger.info("♻️ Resetting BACnet application...")
        try:
            # Tentative de fermeture propre
            if hasattr(_bacnet_app, "close"):
                _bacnet_app.close()
            elif hasattr(_bacnet_app, "shutdown"):
                await _bacnet_app.shutdown()
        except Exception as e:
            logger.error(f"Error closing BACnet app: {e}")
        finally:
            _bacnet_app = None
...
async def acknowledge_alarm(
    ...
    """
    Acquitte une alarme en utilisant automatiquement le timestamp reçu dans le buffer _received_alarms.
    Si aucun timestamp valide n’est trouvé, renvoie une erreur explicite sans planter le serveur.
    """
    try:
        app_bac = await get_bacnet_app()
        remote = Address(f"{host}:{port}")
        ...
        # 🔎 Chercher un timestamp dans le buffer des alarmes
        ts: Optional[TimeStamp] = None
        for alarm in reversed(_received_alarms):  # on prend la plus récente
            if alarm.get("eventObjectIdentifier") == event_object_identifier:
                raw_ts = alarm.get("raw_timestamp") or alarm.get("timestamp_value")
                if isinstance(raw_ts, TimeStamp):
                    ts = raw_ts
                elif raw_ts:
                    try:
                        ts = TimeStamp(dateTime=datetime.fromisoformat(str(raw_ts)))
                    except Exception:
                        continue
                break
        ...
        if ts is None:
            return {
                "status": "error",
                "message": f"Aucun timestamp valide trouvé pour {event_object_identifier}. Impossible d'acquitter."
            }
...
@mcp.tool
async def get_received_alarms(
    kind: Optional[str] = None,     # None, "event", "cov"
    contains: Optional[str] = None, # filtre texte sur message/object/source
    limit: int = 100,
) -> List[Dict[str, Any]]:
    """
    Retourne les dernières alarmes/COV reçues, avec filtres simples.
    - kind: "event" ou "cov" pour filtrer, sinon None pour tout.
    - contains: texte à chercher dans messageText, eventObjectIdentifier, source.
    - limit: nombre max de lignes retournées (du plus récent au plus ancien).
    """
    out: List[Dict[str, Any]] = []
    needle = (contains or "").lower().strip()

    for item in reversed(_received_alarms):
        if kind and item.get("type") != kind:
            continue
        if needle:
            blob = f"{item.get('messageText','')} {item.get('eventObjectIdentifier','')} {item.get('source','')}".lower()
    ...
    return out or [{"status": "Aucune alarme/notification ne correspond au filtre."}]
...
# Sous-application pour les endpoints de supervision / statiques
service_app = FastAPI(title='BACnet MCP service', version=__version__)
...
def _prefixed_health():
    return {'status': 'ok', 'version': __version__}

# (optionnel) servir /static si le dossier existe
try:
    service_app.mount('/static', StaticFiles(directory='static'), name='static')
except Exception:
    pass

# Monter la sous-application sous /mcp pour reproduire MCP Inspector
app.mount('/mcp', service_app)

async def _root_health(request):
    return JSONResponse({'status': 'ok', 'version': __version__, 'path': '/health'})

app.add_route('/health', _root_health, methods=['GET'])

@service_app.options('/{path:path}')
def _options_passthrough(path: str = ''):
    return Response(status_code=204)

def _root_options(request):
    return Response(status_code=204)

app.add_route('/{path:path}', _root_options, methods=['OPTIONS'])
