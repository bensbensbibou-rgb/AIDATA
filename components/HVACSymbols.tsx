...
export const HVACSymbol: React.FC<HVACSymbolProps> = ({
    ...
    const renderSymbol = () => {
        ...
        switch (type) {
            ...
            case 'pump_vacuum': {
                // Extended overlay based on structured value (command/feedback/alarms/manual/display)
                // Si value est un tableau de booléens [cmd, fb, alarm, manuel], on le mappe
                let valObj: any = {};
                ...
                const cmdActive = !!(valObj.command ?? valObj.cd_et ?? valObj.pmp_cd_et);
                const fbActive = !!(valObj.feedback ?? valObj.rm_et ?? valObj.pmp_rm_et);
                const manual = !!(valObj.manual ?? valObj.pmp_mode_man);
                const hasAlarm = !!(valObj.alarm ?? valObj.pmp_th_al ?? valObj.pmp_df_al ?? valObj.pmp_dis_al);
                const maintenance = !!(valObj.maintenance ?? valObj.maint ?? valObj.isMaintenance);
                const masked = !!(valObj.masked ?? valObj.isMasked ?? valObj.mute ?? valObj.silenced);
                const displayVal = valObj.display ?? valObj.value ?? (typeof value === 'number' ? value : undefined);

                // Pilote l'état principal Ã  partir des flags, pas de la valeur affichÃ©e (pour Ã©viter le vert si display=24Â°C)
                if (hasAlarm) state = 'alarm';
                else if (fbActive || cmdActive) state = 'on';
                else state = 'off';

                const statusColor = masked ? '#f472b6' : (maintenance ? '#a855f7' : null);
                const pumpFill = statusColor ? statusColor : getFillColor();
                const pumpStroke = statusColor ? statusColor : getStrokeColor();

                return (
                    <g>
                        {/* Outer ring for feedback/alarm or status */}
                        <circle
                            cx="50" cy="50" r="42"
                            fill="none"
                            stroke={statusColor || (hasAlarm ? '#ef4444' : (fbActive ? '#22c55e' : '#e5e7eb'))}
                            strokeWidth={statusColor ? 6 : (hasAlarm ? 6 : 4)}
                            opacity={statusColor ? 0.9 : (hasAlarm || fbActive ? 1 : 0.6)}
                        />
                        {/* Body */}
                        <circle cx="50" cy="50" r="36" fill="white" stroke={pumpStroke} strokeWidth={sw} />
                        <path
                            d="M30 70 L50 25 L70 70 Z"
                            fill={pumpFill}
                            stroke={pumpStroke}
                            strokeWidth={sw}
                            style={isAnimated && (fbActive || cmdActive) ? { transformOrigin: 'center', animation: 'pulse 2s ease-in-out infinite' } : {}}
                        />
                        {type === 'pump_circulator' && (
                            <circle cx="50" cy="50" r="12" fill="white" stroke={pumpStroke} strokeWidth={sw} />
                        )}
                        {type === 'pump_vacuum' && (
                            <text x="50" y="95" textAnchor="middle" fontSize="16" fontWeight="bold" fill={pumpStroke}>VAC</text>
                        )}
                        {/* Command indicator */}
                        {cmdActive && (
                            <circle cx="50" cy="50" r="10" fill="#22c55e" opacity={0.8} />
                        )}
                        {/* Manual hand badge (visible à droite) */}
                        {manual && (
                            <g transform="translate(84 54) scale(2.0)">
                                <rect x="-10" y="-10" width="20" height="20" rx="10" fill="#0ea5e9" opacity={0.95} />
                                <path d="M-6 6 L-6 -6 L-3 -6 L-3 0 L-1 0 L-1 -7 L2 -7 L2 0 L4 0 L4 -5 L7 -5 L7 6 Z" fill="white" />
                            </g>
                        )}
                        {/* Maintenance wrench badge */}
                        {maintenance && (
                            <g transform="translate(84 18) scale(1.4)">
                                <circle cx="0" cy="0" r="9" fill="#a855f7" opacity={0.95} />
                                <path d="M-5 -1 L-2 -4 L2 -1 L-2 2 Z" fill="white" />
                                <rect x="-1" y="1" width="2" height="8" rx="1" fill="white" />
                            </g>
                        )}
                        {/* Masked overlay */}
                        {masked && <rect x="-4" y="-4" width="108" height="108" fill="#f472b6" opacity="0.22" rx="14" />}
                        {maintenance && !masked && <rect x="-4" y="-4" width="108" height="108" fill="#a855f7" opacity="0.12" rx="14" />}
                        {/* Display value */}
                        {displayVal !== undefined && (
                            <text x="50" y="98" textAnchor="middle" fontSize="12" fill="#374151" fontWeight="bold">
                                {typeof displayVal === 'number' ? displayVal : String(displayVal)}{unit ? ` ${unit}` : ''}
                            </text>
                        )}
                    </g>
                );
            ...
            case 'boiler_buderus':
                return (
                    <g>
                        {/* Blue casing */}
                        <rect x="15" y="20" width="70" height="60" rx="4" fill="#2563eb" stroke={stroke} strokeWidth={sw} />
                        {/* Control panel top */}
                        <rect x="40" y="10" width="20" height="10" fill="#1e40af" stroke={stroke} strokeWidth={sw} />
                        {/* Burner window */}
                        <rect x="25" y="40" width="50" height="20" rx="2" fill="#333" stroke={stroke} strokeWidth="1" />
                        {/* Flame/Glow animation */}
                        <rect
                            x="27" y="42" width="46" height="16" rx="1"
                            fill={state === 'on' ? 'url(#glow-orange)' : '#444'}
                            className={state === 'on' ? 'animate-pulse' : ''}
                        />
                        {/* Pipes */}
                        <line x1="5" y1="50" x2="15" y2="50" stroke={stroke} strokeWidth={sw} />
                        <line x1="85" y1="50" x2="95" y2="50" stroke={stroke} strokeWidth={sw} />
            ...
            case 'boiler_camus_dynamax':
                return (
                    <g>
                        {/* Tall vertical structure */}
                        <rect x="30" y="10" width="40" height="80" fill="#e5e7eb" stroke={stroke} strokeWidth={sw} />
                        {/* Black top cap */}
                        <rect x="30" y="10" width="40" height="15" fill="#1f2937" stroke={stroke} strokeWidth={sw} />
                        {/* Vertical burner chamber */}
                        <rect x="40" y="30" width="20" height="50" fill="#333" stroke={stroke} strokeWidth="1" />
                        {/* Flame */}
                        <rect
                            x="42" y="32" width="16" height="46"
                            fill={state === 'on' ? 'url(#glow-vertical)' : '#444'}
                            className={state === 'on' ? 'animate-pulse' : ''}
                        />
                        {/* Side pipes */}
                        <line x1="70" y1="30" x2="80" y2="30" stroke={stroke} strokeWidth={sw} />
                        <line x1="70" y1="60" x2="80" y2="60" stroke={stroke} strokeWidth={sw} />

                        <defs>
                            <linearGradient id="glow-vertical" x1="0%" y1="0%" x2="0%" y2="100%">
                                <stop offset="0%" stopColor="#f97316" />
            ...
            case 'boiler_cleaver':
                return (
                    <g>
                        {/* Horizontal cylindrical body (Cyan/Blue) */}
                        <rect x="10" y="30" width="80" height="40" rx="20" fill="#06b6d4" stroke={stroke} strokeWidth={sw} />
                        {/* Burner window center */}
                        <rect x="30" y="40" width="40" height="20" rx="2" fill="#333" stroke={stroke} strokeWidth="1" />
                        <rect
                            x="32" y="42" width="36" height="16" rx="1"
                            fill={state === 'on' ? '#f97316' : '#444'}
                            className={state === 'on' ? 'animate-pulse' : ''}
                        />
                        {/* Top stack */}
                        <rect x="25" y="20" width="10" height="10" fill="#06b6d4" stroke={stroke} strokeWidth={sw} />
                        {/* Pipes */}
                        <line x1="90" y1="40" x2="95" y2="40" stroke={stroke} strokeWidth={sw} />
                        <line x1="90" y1="60" x2="95" y2="60" stroke={stroke} strokeWidth={sw} />
                    </g>
                );

            case 'boiler_crest_condensing':
                return (
                    <g>
                        {/* Beige/Tan body */}
                        <path d="M20 20 L80 20 L80 90 L20 90 Z" fill="#d6d3d1" stroke={stroke} strokeWidth={sw} />
                        {/* Angled top left */}
                        <path d="M20 20 L20 30 L30 30 L30 20 Z" fill="#a8a29e" stroke="none" />
                        {/* Internal chamber cutout */}
                        <rect x="45" y="35" width="25" height="45" fill="#333" stroke={stroke} strokeWidth="1" />
                        {/* Flame */}
                        <rect
                            x="47" y="37" width="21" height="41"
                            fill={state === 'on' ? '#f97316' : '#444'}
                            className={state === 'on' ? 'animate-pulse' : ''}
                        />
                        {/* Exhaust pipe curve */}
                        <path d="M35 20 Q35 35 45 35" fill="none" stroke="#57534e" strokeWidth="4" />
                        {/* Side pipes */}
                        <line x1="80" y1="40" x2="90" y2="40" stroke={stroke} strokeWidth={sw} />
                        <line x1="80" y1="70" x2="90" y2="70" stroke={stroke} strokeWidth={sw} />
                    </g>
                );
            ...
            case 'boiler_fulton':
                return (
                    <g>
                        {/* Vertical box, dark grey/black frame */}
                        <rect x="25" y="10" width="50" height="80" fill="#f3f4f6" stroke="#1f2937" strokeWidth="3" />
                        {/* Inner window */}
                        <rect x="35" y="30" width="30" height="40" fill="#333" stroke={stroke} strokeWidth="1" />
                        {/* Vertical flame tubes */}
                        <g className={state === 'on' ? 'animate-pulse' : ''}>
                            <line x1="40" y1="32" x2="40" y2="68" stroke={state === 'on' ? '#f97316' : '#444'} strokeWidth="3" />
                            <line x1="45" y1="32" x2="45" y2="68" stroke={state === 'on' ? '#fbbf24' : '#444'} strokeWidth="3" />
                            <line x1="50" y1="32" x2="50" y2="68" stroke={state === 'on' ? '#f97316' : '#444'} strokeWidth="3" />
                            <line x1="55" y1="32" x2="55" y2="68" stroke={state === 'on' ? '#fbbf24' : '#444'} strokeWidth="3" />
                            <line x1="60" y1="32" x2="60" y2="68" stroke={state === 'on' ? '#f97316' : '#444'} strokeWidth="3" />
                        </g>
                        {/* Side pipes */}
                        <line x1="75" y1="30" x2="85" y2="30" stroke={stroke} strokeWidth={sw} />
                        <line x1="75" y1="70" x2="85" y2="70" stroke={stroke} strokeWidth={sw} />
                    </g>
                );
            ...
            case 'boiler_murray':
                return (
                    <g>
                        {/* Green horizontal body */}
                        <rect x="10" y="35" width="80" height="40" fill="#15803d" stroke={stroke} strokeWidth={sw} />
                        {/* Top attachments */}
                        <rect x="20" y="25" width="5" height="10" fill="#15803d" stroke={stroke} strokeWidth="1" />
                        <rect x="50" y="20" width="10" height="15" fill="#15803d" stroke={stroke} strokeWidth="1" />
                        {/* Burner window */}
                        <rect x="25" y="45" width="30" height="20" fill="#333" stroke={stroke} strokeWidth="1" />
                        <rect
                            x="27" y="47" width="26" height="16"
                            fill={state === 'on' ? '#f97316' : '#444'}
                            className={state === 'on' ? 'animate-pulse' : ''}
                        />
                        {/* Red valve detail */}
                        <rect x="65" y="50" width="5" height="10" fill="#dc2626" />
                        {/* Side pipes */}
                        <line x1="90" y1="45" x2="95" y2="45" stroke={stroke} strokeWidth={sw} />
                        <line x1="90" y1="65" x2="95" y2="65" stroke={stroke} strokeWidth={sw} />
                    </g>
                );
