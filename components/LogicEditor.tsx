...
interface LogicEditorProps {
  ...
  variables: { id: string, label: string, value: any }[];
                       ...
                       {blocks.map(b => {
                           ...
                           return (
                               <foreignObject key={b.id} x={b.x} y={b.y} width="140" height={h + 40} className="overflow-visible">
                                   <div 
                                      className={`w-[140px] rounded-lg bg-white dark:bg-[#2c2c2e] shadow-lg border-2 transition-all flex flex-col relative ${isSelected ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-gray-200 dark:border-white/10'}`}
                                      style={{ height: h }}
                                      onMouseDown={(e) => {
                                          e.stopPropagation();
                                          if (!isEditing) return;
