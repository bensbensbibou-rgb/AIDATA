# Script PowerShell simplifie pour deployer sur Raspberry Pi
param(
...
try {
    $null = ssh -o ConnectTimeout=5 "$RaspberryUser@$RaspberryIP" "echo OK" 2>&1
    ...
    Write-Host "Verifiez l'IP, SSH active, et le reseau" -ForegroundColor Yellow
    exit 1
}
...
ssh "$RaspberryUser@$RaspberryIP" "cd $RemoteDir && unzip -o $archiveName && rm $archiveName"
Write-Host "OK" -ForegroundColor Green
...
Write-Host "  Logs:      ssh $RaspberryUser@$RaspberryIP 'cd $RemoteDir; docker-compose logs -f'"
Write-Host "  Restart:   ssh $RaspberryUser@$RaspberryIP 'cd $RemoteDir; docker-compose restart'"
Write-Host "  Stop:      ssh $RaspberryUser@$RaspberryIP 'cd $RemoteDir; docker-compose down'"
Write-Host ""
